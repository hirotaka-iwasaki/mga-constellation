...
### 許可されていること（本サイトで活用可能）
- ファンオリジナルコンテンツへの楽曲情報の組み込み ✅ 対応済み
- 公式サイト/ユニバーサル公式/YouTubeからのジャケット写真の使用 → **実装可能**
- 公式サイト/ユニバーサル公式/YouTubeからのアーティスト写真の使用 → **実装可能**
- 公式サイト/ユニバーサル公式/YouTubeからのライブ写真の使用 → **実装可能**

### 遵守事項（必須）
- [x] 非営利運営（広告なし、収益化なし）
- [x] 著作権表記（フッターに明記）
- [ ] **ジャケット画像の出典明記** - 画像使用時は公式サイトからの取得である旨を表示
- [ ] **ガイドライン準拠表示** - フッター等に公式ガイドラインに従っている旨を追記
...
### Mrs. GREEN APPLE の深掘り、楽曲の考察
- [ ] **インタビュー・発言の収集** - 大森元貴の発言からバンドの哲学・価値観を抽出
- [ ] **アーティスト哲学ドキュメント作成** - LLMに与える前提情報として整理
- [ ] **歌詞データの収集方法検討** - 著作権に配慮した取得方法を調査
- [ ] **歌詞解析アプローチ設計** `[公開:探索]` - LLM活用による楽曲テーマ抽出の方法論
- [ ] **テーマ抽出プロンプト設計** - 楽曲の本質を言語化するプロンプト作成
- [ ] **楽曲連結アルゴリズム** `[公開:探索]` - 類似テーマの楽曲を繋ぐ「考察星座」ロジック
- [ ] **考察表示UI** `[公開:表示]` - 楽曲カードに考察テキストを表示

### UI/UX 改善
- [ ] **カードにジャケット表示** `[公開:表示]` - 公式サイトからアルバムアートを取得・表示（ガイドライン準拠、出典明記必須）
...
### ガイドライン対応（公式許可により実装可能に）
- [ ] **アーティスト写真の活用** `[公開:表示]` - アバウトページやヘッダーに公式アーティスト写真を表示
- [ ] **ライブ写真ギャラリー** `[公開:探索]` - ライブ星座選択時に公式ライブ写真をサムネイル表示
- [ ] **MV/YouTubeサムネイル表示** `[公開:表示]` - 楽曲カードにYouTube公式MVのサムネイルを表示
...
### ジャケット表示
- **画像取得元**: 公式ガイドラインに従い、以下から取得
  - 公式サイト: https://www.mrsgreenapple.com/
  - ユニバーサルミュージック公式: https://www.universal-music.co.jp/mrsgreenapple/
  - オフィシャルYouTubeチャンネル: https://www.youtube.com/@MrsGREENAPPLE
- **出典明記**: 画像使用時は「© Universal Music Japan」等の出典を表示
- **画像最適化**: WebP形式、適切なサイズ（150x150程度）で影響を最小化
- **遅延読み込み**: `loading="lazy"` で初期表示を軽量に
- **検証**: 実装後にPage Speed Insightsでスコア確認、悪化なら無効化

### 公式素材利用時の注意
- **ガイドライン**: https://www.universal-music.co.jp/mrsgreenapple/guide/
- **収益化禁止**: 広告表示やスポンサーシップは不可
- **イメージ保護**: アーティストの意向やイメージを傷つける編集は禁止
- **定期確認**: ガイドラインの更新を定期的にチェックし遵守
